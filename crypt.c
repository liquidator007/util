...
typedef struct INMENSE { unsigned long l,r; } inmense;
typedef struct GREAT { unsigned short l,c,r; } great;

unsigned long bit[33];
void des(inmense,inmense,int*,int,inmense*);
unsigned long getbit(inmense,int,int);
void ks(inmense,int,great*);
void cyfun(unsigned long,great,unsigned long*);
void cryptfich(FILE*,FILE*,char[],int);

/* DES de NBS (Data Encryption Standard, de National Bureau of Standards
   inp: Datos a encriptar, 64 bits (=8 bytes= 2 enteros largos)
   key: Clave de cifrado,  64 bits (=8 bytes= 2 enteros largos)
   newkey: Flag. Pasar direcc. de entero. Valor 1 si clave nueva
	   (hace que se generen las 16 claves maestras. Usar solo la 1¦vez)
   isw: 0=Encriptar, 1=Desencriptar
   out: Datos de salida, 64 bits (=8 bytes= 2 enteros largos)
*/

void des(inmense inp, inmense key, int *newkey, int isw, inmense *out)
{
  static char ip[65]={0,
  ...
  if (*newkey)  /* Calcular las 16 subclaves maestras */
  {
    *newkey=0;
    for (i=1; i<=16; i++) ks(key,i,&kns[i]);
  }
  itmp.r=itmp.l=0L;  /* permutaci¢n inicial */
  for (j=32, k=64; j>=1; j--, k--)
  {
    itmp.r=(itmp.r<<=1) | getbit(inp,ip[j],32);
    itmp.l=(itmp.l<<=1) | getbit(inp,ip[k],32);
  }
  for (i=1; i<=16; i++)  /* 16 etapas */
  {
    ii=(isw==1? 17-i:i);  /* subclaves en orden inverso para desencriptar*/
...
unsigned long getbit(inmense source, int bitno, int nbits)
{
  if (bitno<=nbits)
...
void ks(inmense key, int n, great *kn)
{
  static inmense icd;
  static char ipc1[57]={0,
  ...
  if (n==1 || n==2 || n==9 || n==16) it=1;
    else it=2;   /* hacer uno o dos desplazamientos, segun n */
  for (i=1; i<=it; i++)
  {
...
{
  static char iet[49]={0,
   32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9,10,11,12,13,12,13,14,
   15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,
   28,29,30,31,32, 1};
  static char ipp[33]={0,
  ...
  static char ibin[16]={ 0, 8, 4,12, 2,10, 6,14,
			 1, 9, 5,13, 3,11, 7,15};
  great ie;
  ...
  for (j=1,m=5;j<=4;j++,m++)
  {
    iec[j]=(char)(ietmp1 & 0x3fUL);
    iec[m]=(char)(ietmp2 & 0x3fUL);
    ietmp1>>=6;
    ietmp2>>=6;
  }
  itmp=0L;
...
void cryptfich(FILE *in, FILE *out, char key[],int enc_des)
{
  size_t nitems;
  int new=1;
  inmense i,o,k,s;
  memset(&k,0,sizeof(k));     /* Poner k a ceros binarios */
  strncpy((char*)&k,key,sizeof(k));  /* Cargar la clave en k, 8 caracteres max */
  while (!feof(in))  /* Mientras no fin de fichero */
  {
    memset(&i,0,sizeof(i));
    nitems=fread(&i,1,sizeof(i),in);
    if (nitems>0)
    {
      des(i,k,&new,enc_des,&o);
...
int main(int argc, char *argv[])
{
  ...
  if (argc!=3)
  {
    fprintf(stderr,"Usage: crypt { -c | -d } key\n");
    exit(2);
  }
  if (!strcmp(argv[1],"-c")) op='c';
  else if (!strcmp(argv[1],"-d")) op='d';
  else
  {
    fprintf(stderr,"Invalid Operation\n");
    exit(2);
  }
  cryptfich(stdin,stdout,argv[2],op=='c'? 0:1);
  return 0;
}
